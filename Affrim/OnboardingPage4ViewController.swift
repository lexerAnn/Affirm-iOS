...
class OnboardingPage4ViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        descriptionLabel.text = "Watch your character evolve as you progress through your daily affirmations! Each completed task brings more joy and energy to your buddy's journey."
        ...
        motivationLabel.text = "✨ Your growth journey becomes their adventure! ✨"
        ...
        NSLayoutConstraint.activate([
            ...
            // Continue button
            continueButton.topAnchor.constraint(greaterThanOrEqualTo: motivationLabel.bottomAnchor, constant: 30),
    ...
    private func createCharacterStatesView() -> UIView {
        ...
        let poorState = createCharacterVideoCard(
            videoName: "poor",
            title: "Just Starting",
            description: "When no tasks are completed",
            color: .systemGray
        )
        ...
        let mediumState = createCharacterVideoCard(
            ...
            description: "When some tasks are completed",
            color: .systemOrange
        )
        ...
        let goodState = createCharacterVideoCard(
            videoName: "good",
            title: "Completed All", 
            description: "When all tasks are finished",
            color: .systemGreen
        )
    ...
    private func createCharacterVideoCard(videoName: String, title: String, description: String, color: UIColor) -> UIView {
        ...
        videoContainer.clipsToBounds = true  // This ensures zoomed content is properly clipped
        videoContainer.backgroundColor = .systemGray6
        videoContainer.layer.masksToBounds = true  // Additional clipping insurance
        
        // Try to load and setup video
        if let videoURL = getVideoURL(for: videoName) {
            let player = AVPlayer(url: videoURL)
            let playerLayer = AVPlayerLayer(player: player)
            playerLayer.videoGravity = .resizeAspectFill  // This will crop to fill, zooming in
            playerLayer.frame = CGRect(x: 0, y: 0, width: 70, height: 70)
            playerLayer.cornerRadius = 12
            
            // Add transform to zoom in on the character (scale up by 2x for better visibility)
        ...
        NSLayoutConstraint.activate([
            ...
            videoContainer.widthAnchor.constraint(equalToConstant: 70),
            videoContainer.heightAnchor.constraint(equalToConstant: 70),
    ...
    private func startVideoPlayback() {
        // Update video layer frames, apply zoom, and start playback
        for (index, playerLayer) in videoLayers.enumerated() {
            playerLayer.frame = CGRect(x: 0, y: 0, width: 70, height: 70)
            playerLayer.transform = CATransform3DMakeScale(2.0, 2.0, 1.0)  // Apply 2x zoom
    ...
    override func viewDidLayoutSubviews() {
        ...
        // Update video layer frames and maintain zoom
        videoLayers.forEach { layer in
            layer.frame = CGRect(x: 0, y: 0, width: 70, height: 70)
            layer.transform = CATransform3DMakeScale(2.0, 2.0, 1.0)  // Maintain 2x zoom
        }
    }
